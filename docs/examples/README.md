...
| [`local-theme`](local-theme/) |  | Find and download themes into a project's `.cac/themes` directory. | `cd local-theme`<br>`cac themes list`<br>`cac themes search`<br>`cac themes install THEME --local` |
